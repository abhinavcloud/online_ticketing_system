...
def _insert_hold_reservation(reservation_id: str, user_sub: str, event_id: str, category_id: str,
                            idempotency_key: Optional[str], expires_at: str,
                            seat_uuid_map: Dict[str, str]) -> None:
    """Insert reservation + reservation_seats rows with HOLD status (all-or-none)."""
    conn = _db_conn()
    with conn.cursor() as cur:
        cur.execute("BEGIN;")
        try:
            cur.execute(
                """
                INSERT INTO public.reservations (id, user_id, event_id, category_id, status, idempotency_key, expires_at, created_at, updated_at)
                VALUES (%s, %s, %s, %s, 'HOLD', %s, %s::timestamptz, now(), now())
                """,
                (reservation_id, user_sub, event_id, category_id, idempotency_key, expires_at),
            )

            # Insert each seat under this reservation
            # reservation_seats references public.seats(id)
            for seat_label, seat_uuid in seat_uuid_map.items():
                cur.execute(
                    """
                    INSERT INTO public.reservation_seats (reservation_id, event_id, seat_id, status, created_at, updated_at)
                    VALUES (%s, %s, %s, 'HOLD', now(), now())
                    """,
                    (reservation_id, event_id, seat_uuid),
                )
...
-- KEYS: one key per seat lock
-- ARGV[1] = ttl_seconds
-- ARGV[2] = value to store
local ttl = tonumber(ARGV[1])
local val = ARGV[2]

-- If any key exists, fail
for i=1,#KEYS do
...
def _delete_locks_best_effort(sl, keys: List[str]):
    if not keys:
        return
    try:
        sl.delete(*keys)
...
def handle_reserve(event, context):
    ...
    expires_at = time.strftime("%Y-%m-%dT%H:%M:%S%z", time.localtime(expires_at_epoch))

    # Create deterministic reservation_id for this request (always)
    reservation_id = str(uuid.uuid4())

    # 1) Validate seats are AVAILABLE in DB (and map to seat UUIDs)
    try:
        seat_uuid_map = _validate_seats_available(event_id, category_id, seat_ids)
    except Exception as e:
        # Create FAILED reservation record and return
        try:
            _insert_failed_reservation(reservation_id, user_sub, event_id, category_id, idempotency_key, str(e))
        except Exception:
    ...
    # 2) Atomic lock in cache (all-or-none)
    try:
        sl = _seatlock_cache()
    except Exception as e:
    ...
    # Keys are hash-tagged to avoid CROSSSLOT in Valkey cluster
    lock_keys = [_seat_lock_key(event_id, category_id, sid) for sid in seat_ids]

    # value stored in cache: reservation_id (useful for debug / later release)
    lock_value = reservation_id

    try:
        ok, conflict_key = sl.eval(_LOCK_ALL_OR_NOTHING_LUA, len(lock_keys), *lock_keys, seat_lock_ttl, lock_value)
        ok = int(ok)
        if ok != 1:
            # This means at least one seat was already locked.
    ...
    # 3) Create reservation record in DB (HOLD) + reservation_seats
    try:
        _insert_hold_reservation(reservation_id, user_sub, event_id, category_id, idempotency_key, expires_at, seat_uuid_map)
    except Exception as e:
        # DB write failed after we acquired locks -> release locks and write FAILED reservation
        _delete_locks_best_effort(sl, lock_keys)
        try:
            _insert_failed_reservation(reservation_id, user_sub, event_id, category_id, idempotency_key, f"DB_INSERT_FAILED: {e}")
        except Exception:
            logger.exception("Failed to write FAILED reservation record")
    ...
    try:
        ...
        # Pricing failure should not invalidate seat locks. Still return reservation success with minimal pricing.
...
def handler(event, context):
    ...
    if method == "POST" and path.endswith("/reserveticket"):
