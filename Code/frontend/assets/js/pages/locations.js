...
(async function init() {
  ...
  try {
    ...
    target.innerHTML = items.length ? items.map(renderCard).join('') : '<div class="message-box warning"><p class="mb-0">No locations returned.</p></div>';
  } catch (error) {
    target.innerHTML = `<div class="message-box danger"><h3 class="mt-0">Failed to load locations</h3><p class="mb-0">${error.message}</p></div>`;
  }
})();
