# API Design Specifications
---

## Authentications
- User must be able to authenticate via a Federated Login or Username/Password

All APIs are secured using Cognito JWT. After authentication, the client includes Authorization: Bearer <token> with every request. User identity is derived server-side from JWT claims (e.g., sub), so request payloads do not include userId.

---
## Location Selection

- User must be able to select the Location
 
 ```json
 GET v1/location
```

```json
 Response:

{
  "page": 1,
  "pageSize": 10,
  "total": 20,
  "locations": [
    {
      "locationId": "location_001",
      "locationName": "ABC"
    }
  ]
}

```

---

## Browsing, and filtering Events, Performers, and Venue by Location

- User must be able to browse by Events or Performers or Venue for each location
- User must be able to see all the details by
 * Events (DateTime, Performer, Ticket Price by Category, Seat Map if applicable)
 * Performers (Events by Date Time should redirect to specific event, Venue)
 * Venue (Events and Performers by Date Time should redirect to specific event)

### Browse by Performers
```json
GET v1/performers?{location=location_id}
```
```json
Response:

{
  "page": 1,
  "pageSize": 10,
  "total": 50,
  "performers": [
    {
      "performerId": "performer_001",
      "performerName": "XYZ"
    }
  ]
}
```

### Browse by Venue
```json
GET v1/venue?{location=location_id}
```

```json
Response:

{
  "page": 1,
  "pageSize": 10,
  "total": 50,
  "venues": [
    {
      "venueId": "venue_001",
      "venueName": "Venue Name"
    }
  ]
}

```

### Browse by Events (filter by Performers And/Or Venue)

```json
GET v1/events?{location=location_id}&{performer=perform_id}&{venue=venueId}
```

```json
Response:

{
  "page": 1,
  "pageSize": 20,
  "total": 150,
  "events": [
    {
      "eventId": "event_001",
      "eventName": "Event Name",
      "dateTime": "2026-06-03T19:30:00+05:30",
      "category": "Concert",

      "location": {
        "locationId": "location_001",
        "locationName": "ABC"
      },

      "venue": {
        "venueId": "venue_001",
        "venueName": "Venue Name"
      },

      "performers": [
        {
          "performerId": "perf_001",
          "performerName": "Performer Name"
        }
      ]
    }
  ]
}
```

---

## Select an Event and Get Event Details

- User selects an Event and get all the event details

```json
GET v1/event/{eventId}
```

```json
Response

{
  "eventId": "event001",
  "eventName": "Event Name",
  "eventDescription": "Event Description",
  "dateTime": "2026-06-03T19:30:00+05:30",
  "status": "ON_SALE",

  "location": {
    "locationId": "location001",
    "locationName": "Location Name"
  },

  "venue": {
    "venueId": "venue_001",
    "venueName": "Name of the Venue"
  },

  "performers": [
    {
      "performerId": "perf_001",
      "performerName": "Name of the performer"
    }
  ],

  "ticketCategories": [
    {
      "categoryId": "category_001",
      "categoryName": "VIP",
      "price": 3000,
      "currency": "INR",
      "totalTickets": 500,
      "availableTickets": 100,
      "status": "AVAILABLE"
    },
    {
      "categoryId": "category_002",
      "categoryName": "General",
      "price": 500,
      "currency": "INR",
      "totalTickets": 2500,
      "availableTickets": 1500,
      "status": "AVAILABLE"
    }
  ],

  "seatMap": {
    "applicable": true,
    "type": "RESERVED_SEATING",
    "seatEndpoint": "/v1/events/event001/seats"
  }
}

```

---

## User selects a Cateogry and enters the watiting queue

- User selects a category and enters a waiting queue per event id and category

```json
POST /queue/enter
```

```json
Request:

{

  "eventId": "event123",
  "categoryId": "categroy_002"

}
```

```json
Response:

{

  "status": "WAITING",

  "bookingToken": "booking-access-token",

  "sessionId": "sess_456",

  "expiresInSeconds": 600,

  "pollAfterSeconds": 5,

  "message": "Please wait while we get you available seats."

}

"status": ["ALLOWED", "WAITING", "SOLD_OUT"]
```

---

## User selects is allowed to browse avaialable seats

- User Selects is allowed to browse available seats as per event and category

```json
GET /v1/events/{eventId}/seats?category_id=category_002
```

```json
Response:

{
  "eventId": "event001",
  "categoryId": "category_002",
  "asOf": "2026-05-11T09:14:16+05:30",

  "seats": [
    {
      "seatId": "A-01-01",
      "row": "A",
      "number": 1,
      "status": "AVAILABLE"
    },
    {
      "seatId": "Z-10-30",
      "row": "Z",
      "number": 30,
      "status": "BOOKED"
    },
    {
      "seatId": "Z-10-31",
      "row": "Z",
      "number": 31,
      "status": "LOCKED"
      "lockExpiresAt": "2026-05-11T09:20:00+05:30"
    }
  ]
}
```

---

## Reserve Ticket

- User is able to reserve tickets and navigate to Payment Gateway for Payments

```json
POST /reserveTicket
```

```json
Request:


{
  "eventId": "event_123",

  "categoryId": "category_001",

  "seats": [
    {
      "seatId": "A-01-01"
    },
    {
      "seatId": "Z-10-30"
    },
    {
      "seatId": "Z-10-31"
    }
  ]
}


```

```json
Response:

{
  "reservationId": "res_123",
  "status": "SUCCESS |  FAILED",
  "expiresAt": "2026-05-11T09:20:00+05:30",
  "eventId": "event_123",
  "categoryId": "category_001",
  "pricing": {
    "currency": "INR",
    "unitPrice": 3000,
    "quantity": 2,
    "totalAmount": 6000
  },
  "seats": {
    "requested": ["A-01-01", "A-01-02"],
    "locked": ["A-01-01", "A-01-02"],
   
  },
  "nextActions": {
    "canProceedToPayment": true,
    "canCancel": true
  }
}

```
---

## Payment Redirection (Out of Scope)


```json
Rquest:

https://payment-gateway.com/pay?amount=9000&currency=INR&reference=res_123&returnUrl=https://your-frontend.com/payment/return?reservationId=res_123
```

```json
Response:
https://your-frontend.com/payment/return?reservationId=res_123&paymentStatus=success&paymentRef=txn_999
```

---

## Alternate/Faliure Path: If reservation fails or payment fails (OUT OF SCOPE FOR CURRENT IMPLEMENTAION. TTL will handle seat unlock as of now)

- If the reservation or payment fails then call an API to delete the seat lock for those respective seats

```json
POST /v1/reservations/{reservationId}/cancel
```

```json
Request:
{
  "reason": "PAYMENT_FAILED | USER_CANCELLED | TIMEOUT | SYSTEM_ABORT",
  "idempotencyKey": "b7b3a93d-fc9d-4d42-9a5e-2bf9a3f1d2db"
}
```

```json
Response:

{
  "reservationId": "resv_12345",
  "status": "CANCELLED",
  "releasedSeatIds": ["A1", "A2", "A3"],
  "releasedAt": "2026-05-14T05:05:00Z"
}
```
---

## Book Ticket

- User makes a successful payment and has booked a ticket



```json
POST /v1/confirmTicket
```

```json
Request:


{
  "reservationId": "res_123",
  "paymentStatus": "SUCCESS",
  "paymentReference": "txn_999",
  "paidAt": "2026-05-11T19:55:00+05:30"
}

```

```json
Response:

Success:

{
  "reservationId": "res_123",
  "bookingId": "bkg_555",
  "status": "CONFIRMED",
  "confirmedAt": "2026-05-11T19:56:10+05:30",
  "eventId": "event_123",
  "categoryId": "category_001",
  "seatIds": ["A-01-01", "Z-10-30", "Z-10-31"],
  "pricing": {
    "currency": "INR",
    "totalAmount": 9000
  }
}


Failure:

{
  "reservationId": "res_123",
  "status": "CANCELLED",
  "reason": "PAYMENT_FAILED",
  "cancelledAt": "2026-05-11T19:56:10+05:30",
  "eventId": "event_123",
  "seatIds": ["A-01-01", "Z-10-30", "Z-10-31"]
}

Expired:

{
  "error": "RESERVATION_EXPIRED",
  "message": "Reservation res_123 expired. Please reserve again."
}


```

---

## Notify User

- User should get a notification on Email/SMS on successful or failed attempts at booking ticket
Notifcation Event via SES/SNS