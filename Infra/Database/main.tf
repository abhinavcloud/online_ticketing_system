...
#Creating an Ingress rule to port 5432 to Aurora Cluster from Lambda Security Group
...
resource "random_id" "aurora_secret_suffix" {
  byte_length = 2 # 2 bytes => 4 hex chars (e.g., a3f9)
}
...
# Create an IAM Policy for Lambda to acceess secret manager secret

resource "aws_iam_policy" "lambda_allow_secret_manager_connection" {
  name        = "lambda-allow-secret-manager-connection"
  path        = "/"
